...
class LLMTaskReconcilerSpringTest extends AbstractSpringOperatorTest {
  ...
  void should_fail_when_llm_definition_is_not_found() {
    ...
    await().pollInterval(1, TimeUnit.SECONDS)
           ...
           .until(() -> getTaskObject(TestUtil::extractState) == State.FAILED);

    assertThat(podsInNamespace(WORKING_NAMESPACE)).isEmpty();

    final var errorMessage = getTaskObject(TestUtil::extractErrorReason);
  ...
  void should_not_reconcile_tasks_from_unwatched_namespaces() {
    ...
    assertThat(getTaskObject(TestUtil::extractState)).isNull();
  ...
  private void verifyPodStateTriggersReconciliation(final String podPhase,
    ...
    await().pollInterval(500, TimeUnit.MILLISECONDS)
           ...
           .until(() -> state == getTaskObject(TestUtil::extractState));
  }
